...
const Appoinment = () => {
  ...
  const [sloTime,setSlotTime] = useState('')
        ...
        <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4'>
          ...
          <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4'>
            {docSlots.length && docSlots[slotIndex].map((item,index)=>(
              <p onClick={()=>setSlotTime(item.time)} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === sloTime ? 'bg-primary text-white' : 'text-gray-400 border border-gray-300'}`} key={index}>
